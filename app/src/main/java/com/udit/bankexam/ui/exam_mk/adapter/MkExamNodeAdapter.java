...
import com.udit.bankexam.ui.exam_mk.ExamMkReportDataActivity;
...
public class MkExamNodeAdapter extends BaseAdapter {
    ...
    public MkExamNodeAdapter(List<ExamNodeBean> mlist_bean, Context mContext) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        MkExamNodeHolder holder = null;
        if (convertView == null) {
            convertView = LayoutInflater.from(mContext).inflate(R.layout.item_exam_node_mk, null);
            holder = new MkExamNodeHolder();
            ViewUtils.initHolderView(holder, convertView, R.id.class);
            convertView.setTag(holder);
        } else {
            holder = (MkExamNodeHolder) convertView.getTag();
        }
        ...
        //做错的题数
        int err = node.getErrCount();
        //已经做的题数
        int doCount = node.getDoCount();

        int tCount = node.getTCount();


        int okCount = doCount - err;
        if (okCount < 0)
            okCount = 0;
        if (doCount < tCount) {
            //没做的 就是错误的
            int cyCount = tCount - doCount;
            err += cyCount;
        }


        String per = Utils.getPer(okCount + "", tCount + "");


        String s = String.format(mContext.getString(R.string.string_mk_ksqingkuang), tCount + "", okCount + "", err + "", per);

        holder.text_exam_done_num.setText(s);
      /*  holder.text_exam_done_num.setText(node.getDoCount()+"");

        holder.text_exam_max_num.setText(node.getTCount()+"");*/
